...
class WalletAddressesService(
    ...
    suspend fun insertWAD(request: WalletAddressesRequestDTO, userId: Int): Int {
    ...
    suspend fun updateWAD(requestDTO: WalletAddressesRequestDTO, userId: Int): Boolean {
    ...
    suspend fun deleteWAD(wadNum: Int, userId: Int): Boolean {
        val now = LocalDateTime.now()
        val currentDate = now.format(DateTimeFormatter.ofPattern("yyyyMMdd"))
        val currentTime = now.format(DateTimeFormatter.ofPattern("HHmmss"))
